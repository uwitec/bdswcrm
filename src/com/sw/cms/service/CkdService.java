...
public class CkdService {
	...
	public void saveCkd(Ckd ckd, List ckdProducts) {
			...
			for (int i = 0; i < ckdProducts.size(); i++) {
				...
				if (cdkProduct != null) {
					if (!cdkProduct.getProduct_id().equals("")) {
						XsdProduct xsdProduct = (XsdProduct) xsdDao
								.getXsdProductInfo(ckd.getXsd_id(),
										cdkProduct.getProduct_id());
						if (xsdProduct != null) {
		...
		if (ckd.getState().equals("已出库")) {
			...
			this.updateXsdState(ckd.getXsd_id(), "已出库", ckd.getYsfs(),
					ckd.getCx_tel(), ckd.getJob_no(), ckd.getSend_time(),
					ckd.getStore_id(), ckd.getFzr(), ckd.getCk_date());
		}
	}
	...
	public void updateCkd(Ckd ckd, List ckdProducts) {
		...
		if (ckd.getState().equals("已出库")) {
			...
			if (tcblMap != null) {
				...
				ds_ratio = tcblMap.get("ds_ratio") == null ? 0
						: ((Double) tcblMap.get("ds_ratio")).doubleValue();
			}
			...
			if (ckdProducts != null && ckdProducts.size() > 0) {
				for (int i = 0; i < ckdProducts.size(); i++) {
					CkdProduct cdkProduct = (CkdProduct) ckdProducts.get(i);
					if (cdkProduct != null
							&& !cdkProduct.getProduct_id().equals("")) {
						XsdProduct xsdProduct = (XsdProduct) xsdDao
								.getXsdProductInfo(ckd.getXsd_id(),
										cdkProduct.getProduct_id());
						Product product = (Product) productDao
								.getProductById(cdkProduct.getProduct_id());
						...
						if ((xsdProduct.getPrice() / (1 + sd / 100)) < product
								.getLsxj()) {
							ds = ds * ds_ratio / 100;
						}
	...
	public String checkKc(Ckd ckd, List ckdProducts) {
		...
		if (ckdProducts != null && ckdProducts.size() > 0) {
			for (int i = 0; i < ckdProducts.size(); i++) {
				...
				if (ckdProduct != null) {
					if (!ckdProduct.getProduct_id().equals("")
							&& !ckdProduct.getProduct_name().equals("")) {
						...
						if (product.getProp().equals("库存商品")) {
							...
							if (cknums > kcnums) {
								msg += ckdProduct.getProduct_name() + " 当前库存为："
										+ kcnums + "  无法出库，请先调拨\n";
							}
						}
					}
		...
		if (ckdProducts != null && ckdProducts.size() > 0) {
			for (int i = 0; i < ckdProducts.size(); i++) {
				...
				if (ckdProduct != null) {
					if (!ckdProduct.getProduct_id().equals("")
							&& !ckdProduct.getProduct_name().equals("")
							&& ckdProduct.getQz_flag().equals("是")) {
						String product_id = ckdProduct.getProduct_id();
						String[] arryNums = (ckdProduct.getQz_serial_num())
								.split(",");
						...
						if (product.getProp().equals("库存商品")) {
							for (int k = 0; k < arryNums.length; k++) {
								...
								if (is_store) {
									...
									msg += ckdProduct.getProduct_name()
											+ " 序列号为：" + serialNum
											+ " 不存在，请确认后再进行出库处理\n";
								}
							}
	...
	private void saveXssk(Xsd xsd) {
		...
		this.saveAccountDzd(xssk.getSkzh(), xssk.getSkje(), xssk.getCzr(),
				xssk.getJsr(), "销售收款，编号[" + xssk.getId() + "]", xssk.getId());
	}
	...
	private void saveQtzc(Xsd xsd) {
		...
		accountsDao.updateAccountJe(xsd.getSkzh(),
				posTypeDao.getBrushCardfy(xsd.getPos_id(), xsd.getSkje())); // 修改账户金额

		double jyje = 0 - posTypeDao.getBrushCardfy(xsd.getPos_id(),
				xsd.getSkje());
		AccountDzd accountDzd = new AccountDzd();
		...
		if (ckdProducts != null && ckdProducts.size() > 0) {
			for (int i = 0; i < ckdProducts.size(); i++) {
				...
				if (ckdProduct != null) {
					if (!ckdProduct.getProduct_id().equals("")
							&& !ckdProduct.getProduct_name().equals("")) {
						productKcDao.updateProductKc(ckd.getStore_id(),
								ckdProduct.getProduct_id(),
								ckdProduct.getNums());
					}
				}
			}
	 ...
	 *            发货时间
	 */
	private void updateXsdState(String xsd_id, String state, String ysfs,
			String cx_tel, String job_no, String send_time, String store_id,
			String ck_jsr, String ck_date) {
		xsdDao.updateXsdState(xsd_id, state, ysfs, cx_tel, job_no, send_time,
				store_id, ck_jsr, ck_date);
	}
	...
	private void updateSerialNum(Ckd ckd, List ckdProducts) {
		if (ckdProducts != null && ckdProducts.size() > 0) {
			for (int i = 0; i < ckdProducts.size(); i++) {
				...
				if (ckdProduct != null) {
					if (!ckdProduct.getProduct_id().equals("")
							&& !ckdProduct.getQz_serial_num().equals("")) {
						String[] arryNums = (ckdProduct.getQz_serial_num())
								.split(",");
						...
						for (int k = 0; k < arryNums.length; k++) {
							serialNumMng.setProduct_id(ckdProduct
									.getProduct_id());
							serialNumMng.setProduct_name(ckdProduct
									.getProduct_name());
							serialNumMng.setProduct_xh(ckdProduct
									.getProduct_xh());
							...
							serialNumFlow.setClient_name(StaticParamDo
									.getClientNameById(ckd.getClient_name()));
							...
							serialNumFlow.setXsdj(ckdProduct.getPrice()
									+ ckdProduct.getJgtz());
	...
	public void addProductSaleFlow(Xsd xsd, List xsdProducts) {
		if (xsdProducts != null && xsdProducts.size() > 0) {
			for (int i = 0; i < xsdProducts.size(); i++) {
				...
				if (xsdProduct != null
						&& !xsdProduct.getProduct_id().equals("")
						&& !xsdProduct.getProduct_name().equals("")) {
					...
					info.setBhsje(xsdProduct.getSjcj_xj()
							/ (1 + xsdProduct.getSd() / 100));
	 ...
	 * @param lsd
	 */
	private void saveXsfpDkfp(Xsd xsd) {
		XsfpDkfp xsfpDkfp = new XsfpDkfp();

		xsfpDkfp.setCzr(xsd.getCzr());
		xsfpDkfp.setDdje(xsd.getXsdje());
		xsfpDkfp.setFplx(xsd.getFplx());
		xsfpDkfp.setFpxxzy(xsd.getFpxx());
		xsfpDkfp.setJy_date(xsd.getCreatdate());
		xsfpDkfp.setJy_jsr(xsd.getFzr());
		xsfpDkfp.setKhhzh(xsd.getKhhzh());
		xsfpDkfp.setKhmc(StaticParamDo.getClientNameById(xsd.getClient_name()));
		xsfpDkfp.setKpdh(xsd.getKp_dh());
		xsfpDkfp.setKpdz(xsd.getKp_address());
		xsfpDkfp.setKpmc(xsd.getKp_mc());
		xsfpDkfp.setSh(xsd.getSh());
		xsfpDkfp.setState("待开");
		xsfpDkfp.setYw_id(xsd.getId());
		xsfpDkfp.setYw_type("销售单");
