...
public class ExportProductLsKcNumsResult extends ExportXlsTemplate {
	...
	public void writeExcelFile(HttpServletRequest request, WritableSheet sheet) {
		...
		try{
			...
			sheet.addCell(label);			
			label = new Label(2,2,"库存数量",this.getFt_item_center_bold());
			sheet.addCell(label);
			label = new Label(3,2,"零售报价",this.getFt_item_center_bold());
			sheet.addCell(label);
			label = new Label(4,2,"零售限价",this.getFt_item_center_bold());
			sheet.addCell(label);
			...
			sheet.addCell(label);
			label = new Label(8,2,"商品卖点",this.getFt_item_center_bold());
			sheet.addCell(label);
			...
			if(productList != null && productList.size()>0){
				...
				for(int i=0;i<productList.size();i++){
					...
					sheet.addCell(label);			
					label = new Label(2,k,num,this.getFt_item_center());
					sheet.addCell(label);
					label = new Label(3,k,JMath.round(lsbj,2),this.getFt_item_right());
					sheet.addCell(label);
					label = new Label(4,k,JMath.round(lsxj,2),this.getFt_item_right());
					sheet.addCell(label);
					label = new Label(5,k,JMath.round(khcbj,2),this.getFt_item_right());
					sheet.addCell(label);
