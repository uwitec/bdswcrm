 ...
 *
 */
public class ExportProductKcNumsResult extends ExportXlsTemplate {
	...
	public void writeExcelFile(HttpServletRequest request, WritableSheet sheet) {
		...
		try{
			...
			sheet.addCell(label);
			label = new Label(5,2,"零售报价",this.getFt_item_center_bold());
			sheet.addCell(label);
			label = new Label(6,2,"零售限价",this.getFt_item_center_bold());
			sheet.addCell(label);
			label = new Label(7,2,"分销价",this.getFt_item_center_bold());
			sheet.addCell(label);
			label = new Label(8,2,"分销限价",this.getFt_item_center_bold());
			sheet.addCell(label);
			...
			if(productList != null && productList.size()>0){
				...
				for(int i=0;i<productList.size();i++){
					...
					sheet.addCell(label);
					label = new Label(5,k,JMath.round(lsbj,2),this.getFt_item_right());
					sheet.addCell(label);
					label = new Label(6,k,JMath.round(lsxj,2),this.getFt_item_right());
					sheet.addCell(label);
					label = new Label(7,k,JMath.round(fxbj,2),this.getFt_item_right());
					sheet.addCell(label);
