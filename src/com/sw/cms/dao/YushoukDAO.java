...
public class YushoukDAO extends JdbcBaseDAO {
	...
	public double getYushoukByClientId(String client_id){
		...
		String sql = "select (sum(hjje)-sum(jsje)) as ysk from yushouk_list where client_name='" + client_id + "' and round(hjje,2)<>round(jsje,2)";
	...
	public void updateYushoukJsje(YushouToYingshou info){
		...
		if(total < 0){ 
			//如果预收款结算金额小于0，添加新的待结算的预收款
			String sql = "insert into yushouk_list(client_name,hjje,remark) values(?,?,?)";
		...
		String sql = "select * from yushouk_list where client_name='" + info.getClient_name() + "' and round(hjje,2)<>round(jsje,2) order by id";
