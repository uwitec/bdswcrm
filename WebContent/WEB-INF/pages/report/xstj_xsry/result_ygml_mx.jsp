...
<TABLE  align="center" cellSpacing=0 cellPadding=0 width="99%" border=0>
	<TBODY>
		<TR style="BACKGROUND-COLOR: #dcdcdc;height:45;">
		    <TD align="center" width="100%"><font style="FONT-SIZE: 16px"><B>业务员预估毛利明细</B></font><br><%=strCon %></TD>
		</TR>
	</TBODY>
</TABLE>
<BR>
...
if(results != null && results.size()>0){
	...
	for(int i=0;i<results.size();i++){
		...
		double dwygcb = map.get("dwygcb")==null?0:((Double)map.get("dwygcb")).doubleValue(); //单位预估成本
		double ygcb = map.get("ygcb")==null?0:((Double)map.get("ygcb")).doubleValue(); //预估成本
		double ml = bhsje - ygcb;
...
%>
		<TR>
			...
			<TD class=ReportItemMoney nowrap><%=JMath.round(dwygcb,2) %>&nbsp;</TD>
			<TD class=ReportItemMoney nowrap><%=JMath.round(ygcb,2) %>&nbsp;</TD>
...
%>
		<TR>
			...
			<TD class=ReportItemMoney>&nbsp;</TD>
			<TD class=ReportItemMoney nowrap><%=JMath.round(hj_ygcb,2) %>&nbsp;</TD>
			<TD class=ReportItemMoney nowrap><%=JMath.round(hj_ml,2) %>&nbsp;</TD>
			<TD class=ReportItemMoney nowrap><%=JMath.percent(hj_ml,hj_ygcb) %>&nbsp;</TD>
		</TR>
<%
}
%>
...
<table width="99%">
		<tr>
			<td width="70%" height="30">注：预估毛利 = 不含税金额 - 预估成本；预估毛利率 = 预估毛利 / 不含税金额 * 100%；点击单据编号可以查看原始单据。</td>
