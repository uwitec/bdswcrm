...
<table width="100%"  align="center"  class="chart_info" cellpadding="0" cellspacing="0" id="tables">
	...
	<tr>
		...
		<td class="a1">维修时间</td>
		<td class="a2"><input type="text" name="wxrkd.wxrk_date" id="wxrk_date" value="<%=StringUtils.nullToStr(wxrkd.getWxrk_date()) %>" size="15"  class="Wdate" onFocus="WdatePicker()">
		<font color="red">*</font>
		</td>
	</tr>
	<tr>
...
<table width="100%"  align="center" id="wxrkdtable"  class="chart_list" cellpadding="0" cellspacing="0">	
 ...
 %>
	<tr>
		<td class="a2">
			<input type="text" id="product_name_<%=i %>" name="wxrkdProducts[<%=i %>].product_name" value="<%=StringUtils.nullToStr(wxrkdProduct.getProduct_name()) %>" size="35" readonly>
		...
		<td class="a2">
			<input type="text" id="product_xh_<%=i %>" name="wxrkdProducts[<%=i %>].product_xh" value="<%=StringUtils.nullToStr(wxrkdProduct.getProduct_xh()) %>" readonly>
		</td>
	    ...
	    <% String productname=StringUtils.nullToStr(wxrkdProduct.getProduct_name());
	     ...
	     %>
	         <input type="text" id="store_id" name="store_id" value="<%=kf %>" size="7" readonly>
	    </td>
	    <td class="a2"><input type="text" id="nums_<%=i %>" name="wxrkdProducts[<%=i %>].nums" value="<%=StringUtils.nullToStr(wxrkdProduct.getNums()) %>" size="7"></td>
		<td class="a2">
			<input type="text" id="qz_serial_num_<%=i %>" name="wxrkdProducts[<%=i %>].qz_serial_num" value="<%=StringUtils.nullToStr(wxrkdProduct.getQz_serial_num()) %>" readonly>
		    <input type="hidden" id="qz_flag_<%=i %>" name="wxrkdProducts[<%=i %>].qz_flag" value="<%=flag2 %>"><a style="cursor:hand" title="左键点击输入输列号" onclick="openSerialWin('<%=i %>');"><b>...</b></a>&nbsp;
		</td>	
			
		<td class="a2">
		    <input type="text" id="product_remark_<%=i %>" name="wxrkdProducts[<%=i %>].remark" value="<%=StringUtils.nullToStr(wxrkdProduct.getRemark()) %>">
		</td>
	</tr>
<%
}
...
<BR>
<font color="red">注：“草稿”指维修入库单暂存，可修改；“提交”后维修入库单不可修改。</font>
<BR><BR>
</FORM>
</BODY>
</HTML>