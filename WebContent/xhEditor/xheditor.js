...
(function($) {
	...
	var itemColors = [ "#FFFFFF", "#E5E4E4", "#D9D8D8", "#C0BDBD", "#A7A4A4",
			"#8E8A8B", "#827E7F", "#767173", "#5C585A", "#000000", "#FEFCDF",
			"#FEF4C4", "#FEED9B", "#FEE573", "#FFED43", "#F6CC0B", "#E0B800",
			"#C9A601", "#AD8E00", "#8C7301", "#FFDED3", "#FFC4B0", "#FF9D7D",
			"#FF7A4E", "#FF6600", "#E95D00", "#D15502", "#BA4B01", "#A44201",
			"#8D3901", "#FFD2D0", "#FFBAB7", "#FE9A95", "#FF7A73", "#FF483F",
			"#FE2419", "#F10B00", "#D40A00", "#940000", "#6D201B", "#FFDAED",
			"#FFB7DC", "#FFA1D1", "#FF84C3", "#FF57AC", "#FD1289", "#EC0078",
			"#D6006D", "#BB005F", "#9B014F", "#FCD6FE", "#FBBCFF", "#F9A1FE",
			"#F784FE", "#F564FE", "#F546FF", "#F328FF", "#D801E5", "#C001CB",
			"#8F0197", "#E2F0FE", "#C7E2FE", "#ADD5FE", "#92C7FE", "#6EB5FF",
			"#48A2FF", "#2690FE", "#0162F4", "#013ADD", "#0021B0", "#D3FDFF",
			"#ACFAFD", "#7CFAFF", "#4AF7FE", "#1DE6FE", "#01DEFF", "#00CDEC",
			"#01B6DE", "#00A0C2", "#0084A0", "#EDFFCF", "#DFFEAA", "#D1FD88",
			"#BEFA5A", "#A8F32A", "#8FD80A", "#79C101", "#3FA701", "#307F00",
			"#156200", "#D4C89F", "#DAAD88", "#C49578", "#C2877E", "#AC8295",
	...
	var arrFontname = [ "宋体", "黑体", "楷体", "隶书", "幼圆", "微软雅黑", "Arial",
	...
	var htmlLink = '<div>链接地址: <input type="text" id="xhEdtLinkHref" value="" class="text" readonly/></div><div>打开方式: <select id="xhEdtLinkTarget"><option selected="selected" value="">默认</option><option value="_blank">新窗口</option><option value="_self">当前窗口</option><option value="_parent">父窗口</option></select></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="确定" /></div>';
	...
	$.xheditor = function(textarea, options) {
		...
		var _this = this, _text = textarea, _jText = $(_text), _jForm = _jText
				.closest("form"), _jTools, _win, _jWin, _doc, _jDoc;
		var bookmark;
		...
		var idCSS = "xhEdtCSS_" + _this.settings.skin, idContainer = "xhEdt"
				+ xCount + "_container", idTools = "xhEdt" + xCount + "_Tool", idIframeArea = "xhEdt"
				+ xCount + "_iframearea", idIframe = "xhEdt" + xCount
				+ "_iframe";
		var bodyClass = "";
		...
		this.init = function() {
			if ($("#" + idCSS).size() == 0) {
				$("head").append(
						'<link id="' + idCSS
								...
								+ jsURL + "xheditor_skin/"
								+ _this.settings.skin + '/ui.css" />')
			}
			var cw = _this.settings.width || _text.style.width
					|| _jText.width();
			...
			_jText
					.after($('<span id="'
							...
							+ '" class="xhEdt_'
							+ _this.settings.skin
							+ '" style="display:none"><table cellspacing="0" cellpadding="0" class="xhEdtLayout" style="width:'
							+ cw
							+ ";height:"
							+ editHeight
							+ 'px;"><tbody><tr><td id="'
							+ idTools
							+ '" class="xhEdtTool" style="height:'
							+ toolbarHeight
							+ 'px"></td></tr><tr><td id="'
							+ idIframeArea
							+ '" class="xhEdtIframeArea" style="height:'
							+ (editHeight - toolbarHeight)
							+ 'px"><iframe frameborder="0" id="'
							+ idIframe
							+ '" src="javascript:;" style="width:100%;" /></td></tr></tbody></table></span>'));
			...
			try {
				...
				_doc.open();
				_doc
						.write('<html><head><base href="'
								+ baseURL
								+ '"/><meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/><link rel="stylesheet" href="'
								+ jsURL + "xheditor_skin/"
								+ _this.settings.skin + '/iframe.css"/>');
				if (_this.settings.loadCSS) {
					_doc
							.write('<link rel="stylesheet" href="' + _this.settings.loadCSS + '"/>')
				}
				_doc
						.write('</head><body dir="ltr" spellcheck="false" class="editMode' + bodyClass + '"></body></html>');
				_doc.close();
				if (isIE) {
								...
								if (n == "GStart") {
									...
									if (n == "GEnd") {
										...
										if (n == "Separator") {
											...
											sToolHtml += '<td><a href="javascript:;" title="'
													+ tool.t
													+ '" name="'
													+ n
													+ '" class="xhEdtButton xhEdtEnabled"><span class="'
													+ cn + '"/></a></td>';
											if (tool.s) {
			...
			if (_jPanel.size() == 0) {
				_jPanel = $('<div id="xhEdtPanel"></div>').mousedown(
						function(ev) {
							ev.stopPropagation()
						});
				_jCntLine = $('<div id="xhEdtCntLine"><img src="' + jsURL
						+ "xheditor_skin/" + _this.settings.skin
						+ '/img/spacer.gif" /></div>');
		...
		this.unbind = function() {
			...
			_jForm.unbind("submit", _this.getSource).unbind("reset",
					_this.setSource);
			var jpWin = $(window);
			jpWin.unbind("unload", _this.getSource).unbind("beforeunload",
					_this.getSource);
			jpWin.unbind("resize", _this.fixFullHeight);
			_jDoc.unbind("keydown", _this.checkShortCut).unbind("keydown",
					_this.forcePtag)
		};
		...
		this.forcePtag = function(ev) {
			...
			if (ev.keyCode == 13 && !ev.shiftKey) {
				var pNode = _this
						.getParent("p,h1,h2,h3,h4,h5,h6,pre,address,div,li");
		...
		this.getRng = function() {
			...
			try {
				rng = sel.rangeCount > 0 ? sel.getRangeAt(0)
						: (sel.createRange ? sel.createRange() : _doc
								.createRange())
			} catch (ex) {
			}
			if (!rng) {
		...
		this.processHTML = function(sHtml, mode) {
			...
			if (mode == "write") {
				if (_this.settings.keepValue) {
					...
					sHtml = sHtml
							.replace(
									/<(\w+(?:\:\w+)?)(\s+[^>]*?(src|href)\s*=\s*(['"]?)\s*([^'"\s]*)\s*\4[^\/>]*?)(\/?)>/ig,
									saveValue)
				}
				sHtml = sHtml.replace(/<(\/?)del( [^>]+)?>/ig, "<$1strike$2>");
				if (isMozilla) {
					sHtml = sHtml
							.replace(/<(\/?)strong( [^>]+)?>/ig, "<$1b$2>");
					sHtml = sHtml.replace(/<(\/?)em( [^>]+)?>/ig, "<$1i$2>")
				} else {
					if (isSafari) {
						sHtml = sHtml
								.replace(
										/("|;)\s*font-size\s*:\s*([a-z-]+)(;?)/ig,
										function(all, pre, sname, aft) {
											var t, s;
						...
						sHtml = sHtml
								.replace(/<strong( [^>]+)?>/ig, "<span"
										+ appleClass
										+ ' style="font-weight: bold;"$1>');
						sHtml = sHtml.replace(/<em( [^>]+)?>/ig, "<span"
								+ appleClass
								+ ' style="font-style: italic;"$1>');
						sHtml = sHtml.replace(/<u( [^>]+)?>/ig, "<span"
								+ appleClass
								+ ' style="text-decoration: underline;"$1>');
						sHtml = sHtml.replace(/<strike( [^>]+)?>/ig, "<span"
								+ appleClass
								+ ' style="text-decoration: line-through;"$1>');
						sHtml = sHtml.replace(/<\/(strong|em|u|strike)>/ig,
								"</span>");
						sHtml = sHtml
								.replace(
				...
				if (!isSafari) {
					function style2font(all, tag, style, content) {
						...
						if (c) {
							...
							if (rgb = c[1]
								...
								rgb = Number(rgb[1]) * 65536 + Number(rgb[2])
										* 256 + Number(rgb[3]);
								rgb = rgb.toString(16);
								while (rgb.length < 6) {
									rgb = "0" + rgb
					...
					sHtml = sHtml
							.replace(
									/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,
									style2font);
					sHtml = sHtml
							.replace(
									/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig,
									style2font);
					sHtml = sHtml
							.replace(
									/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,
									style2font)
				}
			} else {
				if (_this.settings.keepValue) {
					function restoreValue(all, tag, attrs, n, q, v) {
						attrs = attrs.replace(new RegExp("\\s+" + n
								+ "\\s*=\\s*([\"']?)[^\"'\\s]*\\1", "ig"), " "
								+ n + '="' + v + '"');
						return "<" + tag + attrs + ">"
					}
					sHtml = sHtml
							.replace(
									/<(\w+(?:\:\w+)?)(\s+[^>]*?_xhe_(src|href)\s*=\s*(['"]?)\s*([^'"\s]*)\s*\4[^>]*)>/ig,
									restoreValue)
				}
				if (isSafari) {
					sHtml = sHtml.replace(
					...
					sHtml = sHtml
							.replace(
									/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,
									replaceAppleSpan);
					sHtml = sHtml
							.replace(
									/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig,
									replaceAppleSpan);
					sHtml = sHtml
							.replace(
									/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,
									replaceAppleSpan)
				}
				sHtml = sHtml
						.replace(
								/(\s+)(?:_xhe_|_moz_|_webkit_)[^=]+?\s*=\s*(["']?)[^"'\s>]*\2\s*/ig,
								"$1");
				sHtml = sHtml
						.replace(
								/<(\w+[^>]*?)\s+class="?(?:apple|webkit)\-[^ >]*([^>]*?)>/ig,
								"<$1$2>")
			}
			return sHtml
		};
		...
		this.cleanWord = function(sHtml) {
			if (sHtml.match(/mso-|MsoNormal/i)) {
				sHtml = sHtml
						.replace(
								/<!--([\s\S]*?)-->|<style(\s+[^>]+)?>[\s\S]*?<\/style>/ig,
								"");
				sHtml = sHtml.replace(/<\/?\w+:[^>]*>/ig, "");
				sHtml = sHtml
						.replace(/<(\w+[^>]*?)\s+class="?mso[^ >]*([^>]*?)>/ig,
								"<$1$2>");
				sHtml = sHtml.replace(
						/<(\w+[^>]*?)\s+lang="?[^ \>]*([^>]*?)>/ig, "<$1$2>");
				sHtml = sHtml.replace(
						/<(\w+[^>]*?)\s+align="?left"?([^>]*?)>/ig, "<$1$2>");
				...
				sHtml = sHtml.replace(/<(\w+[^>]*?)\s+style="?"?(\s+|>)/ig,
						"<$1$2");
				sHtml = sHtml.replace(
						/<(\w+[^>]*?)\s+style="?\s*([^">]+)\s*"?([^>]*?)>/ig,
						function(all, attr1, styles, attr2) {
							return "<" + attr1 + ' style="'
									+ styles.replace(/&quot;/ig, "") + '"'
									+ attr2 + ">"
						})
		...
		this.cleanHTML = function(sHtml) {
			...
			if (!_this.settings.internalScript) {
				sHtml = sHtml
						.replace(
								/<(script)(\s+[^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,
								"")
			}
			...
			if (!_this.settings.internalStyle) {
				sHtml = sHtml
						.replace(
								/<(style)(\s+[^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,
								"")
			}
			if (!_this.settings.inlineStyle) {
				sHtml = sHtml
						.replace(
								/<(\w+)(\s+[^>]+?)?\s+style\s*=\s*(["']?)[^"']*?\3(\s+[^>]+?)?>/ig,
								"<$1$2$4>")
			}
			for ( var i = 0; i < 3; i++) {
				sHtml = sHtml
						.replace(
								/<(strong|b|u|del|strike|s|em|i)(?:\s+[^>]+)?>(((?!<\1(\s+[^>]+)?>)([ \t\r\n]|&nbsp;))*?)<\/\1>/ig,
								function(all, tag, content) {
									if (content.match(/&nbsp;/i)) {
			...
			sHtml = sHtml
					.replace(
							/<\/(strong|b|u|strike|em|i)>((?:\s|<br\/?>|&nbsp;)*?)<\1>/ig,
							"$2");
			sHtml = sHtml
					.replace(
							/<(p|div)(?:\s+[^>]+)?>(((?!<\1(?: [^>]+)?>)[\s\S])+?)<\/\1>/ig,
							function(all, tag, content) {
								var temp = content
										.replace(
												/<\/?(span|strong|b|u|strike|em|i)(\s+[^>]+)?>/ig,
												"");
								temp = temp
										.replace(/([ \t\r\n]|&nbsp;)+/ig, "");
								if (temp != "") {
		...
		this.formatXHTML = function(sHtml) {
			...
			while (sHtml) {
				...
				if (!stack.last() || !specialTags[stack.last()]) {
					...
					sHtml = sHtml.replace(/^([\s\S]*?)<\/(?:style|script)>/i,
							function(all, text) {
								results.push(text);
								return ""
							});
			...
			function font2style(all, tag, attrs, content) {
				...
				if (styles) {
					content = '<span style="' + styles + '">' + content
							+ "</span>"
				}
				return content
			}
		...
		this.toggleSource = function(state) {
			...
			_jTools.find("[name=Source]").toggleClass("xhEdtEnabled")
					.toggleClass("xhEdtActive");
			_jTools.find(".xhEdtButton").not(
					"[name=Source],[name=Fullscreen],[name=About]")
					.toggleClass("xhEdtEnabled");
			if (bShowPanel) {
				_this.hidePanel()
			}
			...
			if (!bSource) {
				...
				jBody.removeAttr("scroll")
						.attr("class", "editMode" + bodyClass);
				setTimeout(function() {
					_win.scrollTo(0, 0)
				}, 10)
			}
		...
		this.togglePreview = function(state) {
			...
			_jTools.find("[name=Preview]").toggleClass("xhEdtActive")
					.toggleClass("xhEdtEnabled");
			_jTools.find(".xhEdtButton").not(
					"[name=Preview],[name=Fullscreen],[name=About]")
					.toggleClass("xhEdtEnabled");
			var jBody = $(_doc.body);
			if (!bPreview) {
				if (isIE) {
		...
		this.toggleFullscreen = function(state) {
			...
			var jArea = $("#" + idIframeArea), jLayout = $("#" + idContainer)
					.find(".xhEdtLayout"), jContainer = $("#" + idContainer);
			if (bFullscreen) {
				if ($.boxModel === false) {
		...
		this.checkShortCut = function(event) {
			...
			ev = event;
			var code = ev.which, special = specialKeys[code], sChar = special ? special
		...
		this.showMenu = function(menuitems, callback) {
			...
			$.each(menuitems, function(n, v) {
				jItem = $(
						'<a href="javascript:;" title="' + v.t + '">' + v.s
								+ "</a>").click(function() {
					_this.focus();
					callback(v.v);
					_this.hidePanel()
			...
			$.each(itemColors, function(n, v) {
				...
				jItem = $(
						'<a href="javascript:;" title="' + v
								+ '" style="background:' + v + '"><img src="'
								+ jsURL + "xheditor_skin/"
								+ _this.settings.skin
								+ '/img/spacer.gif" /></a>').click(function() {
					_this.focus();
					callback(v);
					_this.hidePanel()
				}).mousedown(function() {
		...
		this.showPastetext = function() {
			...
			var jValue = $("#xhEdtPastetextValue", jPastetext), jSave = $(
					"#xhEdtSave", jPastetext);
		...
		this.showLink = function() {
			var jLink = $(htmlLink);
			var jParent = _this.getParent("a"), jHref = $("#xhEdtLinkHref",
					jLink), jTarget = $("#xhEdtLinkTarget", jLink), jSave = $(
					"#xhEdtSave", jLink);
			if (jParent.size() == 1) {
			...
			if (_this.settings.upLinkUrl) {
				_this.ajaxUploadInit(jHref, _this.settings.upLinkUrl,
						_this.settings.upLinkExt)
			}
			jSave.click(function() {
				_this.focus();
				var url = jHref.val();
				...
				if (url != "") {
					url="DownLoadFileServlet?file="+url;
					if (jParent.size() == 0) {
		...
		this.showImg = function() {
			var jImg = $(htmlImg);
			var jParent = _this.getParent("img"), jSrc = $("#xhEdtImgSrc", jImg), jAlt = $(
					"#xhEdtImgAlt", jImg), jAlign = $("#xhEdtImgAlign", jImg), jWidth = $(
					"#xhEdtImgWidth", jImg), jHeight = $("#xhEdtImgHeight",
					jImg), jBorder = $("#xhEdtImgBorder", jImg), jVspace = $(
					"#xhEdtImgVspace", jImg), jHspace = $("#xhEdtImgHspace",
					jImg), jSave = $("#xhEdtSave", jImg);
			if (jParent.size() == 1) {
				...
				var vspace = jParent.attr("vspace"), hspace = jParent
						.attr("hspace");
				jVspace.val(vspace < 0 ? 0 : vspace);
				jHspace.val(hspace < 0 ? 0 : hspace)
			}
			if (_this.settings.upImgUrl) {
				_this.ajaxUploadInit(jSrc, _this.settings.upImgUrl,
						_this.settings.upImgExt)
			}
			jSave.click(function() {
				_this.focus();
				var url = jSrc.val();
		...
		this.showFlash = function() {
			...
			var jParent = _this
					.getParent('embed[type="application/x-shockwave-flash"]'), jSrc = $(
					"#xhEdtFlashSrc", jFlash), jWidth = $("#xhEdtFlashWidth",
					jFlash), jHeight = $("#xhEdtFlashHeight", jFlash), jSave = $(
					"#xhEdtSave", jFlash);
			if (jParent.size() == 1) {
				jSrc.val(_this.attr(jParent, "src"));
				jWidth.val(jParent.attr("width"));
			...
			if (_this.settings.upFlashUrl) {
				_this.ajaxUploadInit(jSrc, _this.settings.upFlashUrl,
			...
			jSave
					.click(function() {
						...
						if (sSrc != "") {
							if (jParent.size() == 0) {
								_this
										.pasteHTML('<embed type="application/x-shockwave-flash" src="#xhedt_tempurl" wmode="opaque" quality="high" bgcolor="#ffffff" menu="false" play="true" loop="true" />');
								jParent = $('embed[src$="#xhedt_tempurl"]',
										_doc)
							}
		...
		this.showMeida = function() {
			...
			var jParent = _this
					.getParent('embed[type="application/x-mplayer2"]'), jSrc = $(
					"#xhEdtMediaSrc", jMedia), jWidth = $("#xhEdtMediaWidth",
					jMedia), jHeight = $("#xhEdtMediaHeight", jMedia), jSave = $(
					"#xhEdtSave", jMedia);
			if (jParent.size() == 1) {
				jSrc.val(_this.attr(jParent, "src"));
				jWidth.val(jParent.attr("width"));
			...
			if (_this.settings.upMediaUrl) {
				_this.ajaxUploadInit(jSrc, _this.settings.upMediaUrl,
						_this.settings.upMediaExt)
			}
			jSave
					.click(function() {
						_this.focus();
						...
						if (sSrc != "") {
							if (jParent.size() == 0) {
								_this
										.pasteHTML('<embed type="application/x-mplayer2" src="#xhedt_tempurl" enablecontextmenu="false" autostart="false" />');
								jParent = $('embed[src$="#xhedt_tempurl"]',
										_doc)
							}
							_this.attr(jParent, "src", sSrc);
		...
		this.ajaxUploadInit = function(jText, tourl, upext) {
			...
			if (tourl.substr(0, 1) == "!") {
				...
				$(".btn", jUpload).before(jText).click(function() {
					_this.showModal(tourl.substr(1), "上传文件", function(v) {
						jText.val(v)
					})
				})
			} else {
		...
		this.ajaxUpload = function(fromfile, tourl, callback) {
			...
			var jIO = $('<iframe name="' + idIO + '" class="xhEdtUploadIO" />')
					.appendTo("body");
			var jForm = $(
					'<form action="'
							+ tourl
							+ '" target="'
							+ idIO
							+ '" method="post" enctype="multipart/form-data" class="xhEdtUploadForm"></form>')
					.appendTo("body");
			var jOldFile = $(fromfile), jNewFile = jOldFile.clone().attr(
					"disabled", "true");
			...
			jIO
					.load(function() {
						...
						var strText = $(jIO[0].contentWindow.document.body)
								.text(), data = Object;
						try {
							eval("data=" + strText)
						} catch (ex) {
		...
		this.showModal = function(ifmurl, title, callback, w, h) {
			var jOverlay = $('<div class="xhEdtOverlay"></div>').appendTo(
			...
			var jModal = $(
					'<div class="xhEdtModal" style="width:'
							+ w
							+ "px;height:"
							+ h
							+ "px;margin:-"
							...
							+ Math.ceil(w / 2)
							+ 'px;">'
							+ (_this.settings.modalTitle ? '<div class="xhEdtTitle"><span class="xhEdtClose" title="Close"></span>'
									+ title + "</div>"
									: "")
							+ '<div class="xhEdtContent"></div></div>')
		...
		this.showEmot = function() {
			var jEmot = $('<div class="xhEdtEmot"></div>'), jLine, jItem, c = 0, sEmotPath = jsURL
					+ "xheditor_emot/default/";
			jLine = $("<div></div>");
			$.each(itemEmots, function(n, v) {
				c++;
				jItem = $(
						'<a href="javascript:;" title="' + v.t + '"><img src="'
								+ sEmotPath + v.s + '" /></a>').click(
						function() {
							_this.focus();
							_this.pasteHTML('<img src="' + sEmotPath + v.s
									+ '" alt="' + v.t + '">');
							_this.hidePanel()
						}).mousedown(function() {
					return false
		...
		this.showTable = function() {
			...
			var jRows = $("#xhEdtTableRows", jTable), jColumns = $(
					"#xhEdtTableColumns", jTable), jHeaders = $(
					"#xhEdtTableHeaders", jTable), jWidth = $(
					"#xhEdtTableWidth", jTable), jHeight = $(
					"#xhEdtTableHeight", jTable), jBorder = $(
					"#xhEdtTableBorder", jTable), jCellSpacing = $(
					"#xhEdtTableCellSpacing", jTable), jCellPadding = $(
					"#xhEdtTableCellPadding", jTable), jAlign = $(
					"#xhEdtTableAlign", jTable), jCaption = $(
					"#xhEdtTableCaption", jTable), jSave = $("#xhEdtSave",
					jTable);
			jSave
					.click(function() {
						_this.focus();
						var sCaption = jCaption.val(), sBorder = jBorder.val(), sRows = jRows
								.val(), sCols = jColumns.val(), sHeaders = jHeaders
								.val(), sWidth = jWidth.val(), sHeight = jHeight
								.val(), sCellSpacing = jCellSpacing.val(), sCellPadding = jCellPadding
								.val(), sAlign = jAlign.val();
						var i, j, htmlTable = "<table"
								+ (sBorder != "" ? ' border="' + sBorder + '"'
										: "")
								+ (sWidth != "" ? ' width="' + sWidth + '"'
										: "")
								+ (sHeight != "" ? ' width="' + sHeight + '"'
										: "")
								+ (sCellSpacing != "" ? ' cellspacing="' + sCellSpacing + '"'
										: "")
								+ (sCellPadding != "" ? ' cellpadding="' + sCellPadding + '"'
										: "")
								+ (sAlign != "" ? ' align="' + sAlign + '"'
										: "") + ">";
						if (sCaption != "") {
		...
		this.exec = function(cmd) {
			...
			case "fontsize":
				...
				$.each(arrFontsize, function(n, v) {
					menuFontsize.push( {
						s : '<span style="font-size:' + v.s + '">' + v.t + "("
								+ v.s + ")</span>",
						v : n + 1,
						t : v.t
					})
	...
	$(function() {
		...
		$("textarea.xheditor-mini").xheditor(true, {
			tools : "mini"
		});
		$("textarea.xheditor-simple").xheditor(true, {
			tools : "simple"
		})
	})
})(jQuery);